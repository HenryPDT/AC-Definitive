...
namespace ACB
{
    namespace Scimitar
    {
        ...
        struct PadProxyPC : Pad
        {
            ...
            PadData pads[NbPadSets]; // 0x5BC -> 0x167C
            char _tail_padding[4]; // 0x167C -> 0x1680 (Align to 16 bytes)

            bool AddPad(Pad* a, PadType b, const wchar_t* c, uint16_t d, uint16_t e);
            void Update(); // We hook this
        };
        static_assert(sizeof(PadProxyPC) == 0x1680, "PadProxyPC size mismatch");
    }
}
