...
namespace BaseHook
{
    namespace Hooks
    {
        ...
        bool IsIgnoredWindowClass(const CharT* lpClassName, const CharT* lpWindowName, int nWidth, int nHeight)
        {
            ...
            if (nWidth != (int)0x80000000 && nWidth <= 32 && nWidth >= 0) return true;
            ...
            if (lpClassName && !IS_INTRESOURCE(lpClassName)) {
                if constexpr (std::is_same_v<CharT, char>) {
                    ...
                    if (contains(lpClassName, "D3DProxyWindow")) return true;
                }
                else {
                    ...
                    if (contains(lpClassName, L"D3DProxyWindow")) return true;
                }
            }
            ...
            if (lpWindowName && !IS_INTRESOURCE(lpWindowName)) {
                if constexpr (std::is_same_v<CharT, char>) {
                    ...
                    if (equals(lpWindowName, "Kiero DirectX Window")) return true;
                }
                else {
                    if (contains(lpWindowName, L"DIEmWin")) return true;
                    if (contains(lpWindowName, L"Default IME")) return true;
                    if (equals(lpWindowName, L"Kiero DirectX Window")) return true;
                }
            }
