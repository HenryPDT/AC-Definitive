...
namespace BaseHook
{
    namespace WindowedMode
    {
        ...
        static bool QueryDXGIExclusiveFullscreen(bool& outExclusive)
        {
            outExclusive = false;
            if (!Data::pSwapChain)
                return false;
        ...
        bool IsActuallyExclusiveFullscreen()
        {
            ...
            if (!QueryDXGIExclusiveFullscreen(exclusive))
                return false;
            return exclusive;
        }
        ...
        void TickDXGIState()
        {
            ...
            bool exclusive = false;
            const bool haveState = QueryDXGIExclusiveFullscreen(exclusive);
            if (haveState)
            {
        ...
        void Apply(HWND hWnd)
        {
            ...
            // LOG_INFO("WindowedMode Applied to HWND %p (Changes: Style=%d, Pos=%d)", hWnd, styleChanged, posChanged);
        }
        ...
        void NotifyResolutionChange(int width, int height)
        {
            ...
            if (g_State.resizeBehavior == ResizeBehavior::ResizeWindow && g_State.activeMode != Mode::BorderlessFullscreen)
            {
        ...
        bool PhysicalClientToVirtual(HWND hWnd, POINT& pt)
        {
            ...
            pt.x = (LONG)((float)pt.x * ((float)g_State.virtualWidth / (float)clientW));
            pt.y = (LONG)((float)pt.y * ((float)g_State.virtualHeight / (float)clientH));
            return true;
        }
        ...
        bool VirtualClientToPhysical(HWND hWnd, POINT& pt)
        {
            ...
            pt.x = (LONG)((float)pt.x * ((float)clientW / (float)g_State.virtualWidth));
            pt.y = (LONG)((float)pt.y * ((float)clientH / (float)g_State.virtualHeight));
            return true;
        }

        LPARAM ScaleMouseMessage(HWND hWnd, UINT uMsg, LPARAM lParam)
        {
            if ((!ShouldHandle() && !ShouldApplyResolutionOverride()) || hWnd != g_State.hWnd) return lParam;
