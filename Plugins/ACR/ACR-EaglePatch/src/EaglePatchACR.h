...
namespace ACREaglePatch
{
    ...
    namespace scimitar
    {
        ...
        struct Pad
        {
            ...
            char _pad_align[0x28];    // 0x588 -> 0x5B0 (Sizeof Pad)
        ...
        static_assert(sizeof(Pad) == 0x5B0, "Pad size mismatch");
        ...
        // PadXenon adds 0x20 bytes (0x5B0 -> 0x5D0)
        static_assert(sizeof(PadXenon) == 0x5D0, "PadXenon size mismatch");

        struct PadData
        {
            scimitar::Pad* pad;           // 0x000
            char field_4[528];            // 0x004
            InputBindings* pInputBindings; // 0x214
            char _extra_padding[20];      // 0x218 -> 0x22C (ACR has larger PadData than ACB)
        };
        // 4 + 528 + 4 + 20 = 556 (0x22C) - ACR stride confirmed by assembly

        enum PadSets {
            Keyboard1 = 0, Keyboard2, Keyboard3, Keyboard4, Keyboard5,
            Joy1, Joy2, Joy3,
            NbPadSets
        };
        ...
        {
            // Pad is 0x5B0
            int field_5B0;        // 0x5B0 -> 0x5B4
            uint32_t selectedPad; // 0x5B4 -> 0x5B8
            int field_5B8;        // 0x5B8 -> 0x5BC
            
            scimitar::PadData pads[NbPadSets]; // 0x5BC -> 0x171C (ACR: 8 * 0x22C = 0x1160)
            char _tail_padding[0x84]; // 0x171C -> 0x17A0 (Align, larger than ACB due to bigger PadData)

            bool AddPad(scimitar::Pad* a, PadType b, const wchar_t* c, uint16_t d, uint16_t e, int64_t f, int64_t g);
            void Update(); // We hook this
        };
        static_assert(sizeof(PadProxyPC) == 0x17A0, "PadProxyPC size mismatch");
    }
...
extern ACREaglePatch::t_ac_getNewDescriptor ac_getNewDescriptor;
extern ACREaglePatch::t_ac_getDeleteDescriptor ac_getDeleteDescriptor;