...
{
    if (ImGui::CollapsingHeader("Character Model", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ImGui::TextWrapped("Select a character model. Fast Travel to apply.");
        ...
        if (prog)
        {
            ...
            ImGui::Text("Active: %s", currName.c_str());

            if (ImGui::BeginListBox("##Models", ImVec2(-FLT_MIN, 150)))
            {
